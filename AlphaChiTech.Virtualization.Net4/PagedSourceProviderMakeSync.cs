...
namespace AlphaChiTech.Virtualization
{
    public class PagedSourceProviderMakeSync<T> : IPagedSourceProviderAsync<T>, IProviderPreReset
    {
        ...
        public int Count
        {
            get { return Task.Factory.Run(() => GetCountAsync()).Result; }
        }
        ...
        public virtual int IndexOf(T item)
        {
            ...
            {
                ret = Task.Factory.Run( () => _FuncIndexOfAsync.Invoke(item)).Result;
            } 
            else
            {
                ret = Task.Factory.Run(() => IndexOfAsync(item)).Result;
            }
